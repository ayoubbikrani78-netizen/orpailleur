...
export default function Commandes() {
  ...
  async function fetchAll() {
    const { data: c } = await supabase
      .from('commandes')
      .select('*, fournisseurs(nom, franco_type, franco_valeur, frais_port_type, frais_port_montant, frais_port_texte)')
...
async function deleteCommande() {
  ...
  function envoyerEmail() {
    const f = selected.fournisseurs
    const totaux = calculerTotaux()
    const corps = lignes.map(l => `${l.matieres_premieres_fournisseurs?.designation_fournisseur} : ${l.quantite_commandee} unités`).join('%0D%0A')
    ...
    window.location.href = `mailto:${f?.email}?subject=${sujet}&body=${body}`
  }
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
            ...
            {!showNewFournisseur ? (
              <>
                ...
                <button onClick={() => setShowNewFournisseur(true)} className="w-full px-4 py-2.5 rounded-lg border border-dashed border-gray-300 text-sm font-medium text-gray-500 hover:border-yellow-400 hover:text-yellow-600">
                  + Nouveau fournisseur
                </button>
              </>
            ) : (
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowNewFournisseur(false)} className="flex-1 py-2 text-sm text-gray-500">Annuler</button>
...
function CommandeDetail({ commande, lignes, calculerTotaux, onClose, onUpdateQuantite, onEnvoyer, onExportPDF, onEnvoyerEmail, onDelete }) {
  const totaux = calculerTotaux()
  const isEditable = commande.statut === 'brouillon'

  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8 print:hidden">
        <div className="flex items-center justify-between mb-6">
          <div>
...
{lignes.length === 0 ? (
          <div className="text-center py-8 mb-6 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-500 mb-3">Ce fournisseur n'a aucun produit lié dans la mercuriale.</p>
      ...
      <div className="hidden print:block print:fixed print:inset-0 print:bg-white print:p-12">
        <div className="flex justify-between items-start mb-10">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#C9A84C' }}>Orpailleur</h1>
            <p className="text-xs text-gray-400">Bon de commande</p>
          </div>
        ...
        <div className="mb-8">
          <p className="text-xs text-gray-400 uppercase mb-1">Fournisseur</p>
          <p className="font-bold text-lg text-gray-800">{commande.fournisseurs?.nom}</p>
        </div>

        <table className="w-full text-sm mb-8">
          <thead>
            <tr className="border-b-2" style={{ borderColor: '#C9A84C' }}>
              <th className="text-left py-2 font-semibold text-gray-600">Référence</th>
              <th className="text-left py-2 font-semibold text-gray-600">Désignation</th>
              <th className="text-right py-2 font-semibold text-gray-600">Quantité</th>
              <th className="text-right py-2 font-semibold text-gray-600">Prix unit. HT</th>
              <th className="text-right py-2 font-semibold text-gray-600">Total HT</th>
            </tr>
          </thead>
          <tbody>
            {lignes.map(l => (
              <tr key={l.id} className="border-b border-gray-100">
                <td className="py-3 text-gray-500">{l.matieres_premieres_fournisseurs?.reference_fournisseur}</td>
                <td className="py-3 text-gray-700">{l.matieres_premieres_fournisseurs?.designation_fournisseur}</td>
                <td className="py-3 text-right text-gray-700">{l.quantite_commandee}</td>
                <td className="py-3 text-right text-gray-700">{parseFloat(l.prix_unitaire_ht || 0).toFixed(2)}€</td>
                <td className="py-3 text-right font-medium text-gray-800">{parseFloat(l.montant_ht || 0).toFixed(2)}€</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end">
          <div className="w-64 space-y-2">
            ...
            <div className="flex justify-between font-bold text-gray-800 pt-2 border-t" style={{ borderColor: '#C9A84C' }}>
